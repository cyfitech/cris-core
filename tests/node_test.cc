...
TEST(NodeTest, Basic) {
...

    {
        ...
        subscriber.Subscribe<TestMessage<10086>>(channel_subid, [](const auto&) {
            // Never enters.
            std::abort();
        });

        // Different subid, never called
        subscriber.Subscribe<TestMessageType>(10010, [](const auto&) {
            // Never enters.
            std::abort();
        });
        ...
        subscriber.Subscribe<TestMessageType>(
            channel_subid,
            [message_value, cv, received](const std::shared_ptr<TestMessageType>& msg) {
        ...
        subscriber.Subscribe<TestMessageType>(channel_subid, [&](const auto&) {
            // Never enters.
            std::abort();
        });
        ...
        cv->wait(cv_lock, [received]() { return received->load(); });
    }
...
TEST(NodeTest, MultipleChannels) {
    ...
    for (std::size_t channel_subid = 0; channel_subid < kNumOfSubChannel; ++channel_subid) {
        subscriber.Subscribe<TestMessage<0>>(
            channel_subid,
            [received, channel_subid, cv](const std::shared_ptr<TestMessage<0>>& msg) {
                received->data_[0][channel_subid].store(msg->value_);
                cv->notify_all();
            });
        subscriber.Subscribe<TestMessage<1>>(
            channel_subid,
            [received, channel_subid, cv](const std::shared_ptr<TestMessage<1>>& msg) {
                received->data_[1][channel_subid].store(msg->value_);
                cv->notify_all();
            });
        subscriber.Subscribe<TestMessage<2>>(
            channel_subid,
            [received, channel_subid, cv](const std::shared_ptr<TestMessage<2>>& msg) {
                received->data_[2][channel_subid].store(msg->value_);
                cv->notify_all();
            });
        subscriber.Subscribe<TestMessage<3>>(
            channel_subid,
            [received, channel_subid, cv](const std::shared_ptr<TestMessage<3>>& msg) {
                received->data_[3][channel_subid].store(msg->value_);
                cv->notify_all();
            });
    }
    ...
    for (std::size_t channel_subid = 0; channel_subid < kNumOfSubChannel; ++channel_subid) {
        {
            ...
            EXPECT_EQ(received->data_[message_type_idx][channel_subid].load(), 0);
            ...
            cv->wait(cv_lock, [received, channel_subid]() {
                return received->data_[message_type_idx][channel_subid].load() != 0;
            });

            EXPECT_EQ(received->data_[message_type_idx][channel_subid].load(), message_value);
        }
