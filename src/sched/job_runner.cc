...
//
// Safety:
//   - There are no more than one jobs from the same strand in the bound runner;
...
bool JobRunnerStrand::AddJob(JobRunnerStrand::job_t&& job) {
    ...
    //   - D/d+ only changes the has_ready_job_ value from false to true
    //   - D/d- only changes the has_ready_job_ value from true to false
    //   - P and D/d- has one-to-one matching and P always happen before the correspoding D/d
    //   - D/d generates a P if and only if has_ready_job_ is true when exiting from D
    ...
    auto try_to_run_next_job = [](JobRunnerStrand& strand, bool is_in_running_job) {
        std::unique_ptr<job_t> next(nullptr);

        {
            std::unique_lock lock(strand.hybrid_spin_mtx_);

            // Someone else is running or about to run a job.
            if (!is_in_running_job && strand.has_ready_job_) {
                return;
            }

            strand.pending_jobs_.consume_one([&next](job_t* const job_ptr) { next.reset(job_ptr); });

            if (!next) {
                strand.has_ready_job_ = false;
                return;
            }
            strand.has_ready_job_ = true;
        }
    ...
    auto serialized_job = [job = std::move(job), strand_weak = weak_from_this(), try_to_run_next_job] {
        job();

        if (auto strand = strand_weak.lock()) {
            try_to_run_next_job(*strand, /* is_in_running_job = */ true);
        }
    };

    pending_jobs_.push(new job_t(std::move(serialized_job)));
    try_to_run_next_job(*this, /* is_in_running_job = */ false);
    return true;
}
